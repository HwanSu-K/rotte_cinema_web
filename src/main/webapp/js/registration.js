$(document).ready(function() {
	...
	$('#year,#month').on('change', function() {
		if ($('#year option:selected').val() === "" || $('#month option:selected').val() === "") {
			return false;
		}

		var day = fn_DayOfMonth($('#year option:selected').val(), $('#month option:selected').val());
	...
	$('#password,#passwordcheck').on('blur', function() {
		...
		if (pw.length < 8) {
			...
			$('#passAlert').text('비밀번호는 8자리 이상으로 입력해주세요.');
			return false;
		} else if (pw.search(/\s/) != -1) {
			$('#passAlert').removeClass('disabled');
			$('#passAlert').text('비밀번호는 공백 없이 입력해주세요.');
			return false;
		} else if (num < 0 || eng < 0 || spe < 0) {
			$('#passAlert').removeClass('disabled');
			$('#passAlert').text('영문,숫자,특수문자를 혼합하여 입력해주세요.');
			return false;
		}
	...
	$('#btn_reg').on('click', function(e) {
		...
		if (input) {
			// 비밀번호 불일치시 진행 안함.
			if(!$('#passAlert').hasClass('disabled'))
			{
				return false;
			}

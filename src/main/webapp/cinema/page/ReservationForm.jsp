...
<body>
	...
	<div class="content">
      ...
      <div class="reserv_screen">
		<div class="mask">
			<div><i class="fas fa-arrow-up"></i><span>인원 선택후 좌석을 선택해 주세요.</span><i class="fas fa-arrow-up"></i></div>
		</div>
        ...
        <c:forEach begin="1" end="${reservations.theaterSeatY }" varStatus="statusY">
        	<div>
        		<span>${array[statusY.index - 1]}</span>
        		<c:forEach begin="1" end="${reservations.theaterSeatX }" varStatus="statusX">	
		        	<div id="x${statusX.index }y${statusY.index }" data-seat-x="${statusX.index }" data-seat-y="${statusY.index }">${statusX.index }</div>
		        </c:forEach>
        	</div>
        </c:forEach>
        </div>
